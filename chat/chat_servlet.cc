...
CatChatWSServlet::CatChatWSServlet()
    :yk::http::WSServlet("cat_chat_servlet") {
}

int32_t CatChatWSServlet::onConnect(yk::http::HttpRequest::ptr header
                              ,yk::http::WSSession::ptr session) {
    YK_LOG_INFO(g_logger) << "onConnect ";
    return 0;
}

int32_t CatChatWSServlet::onClose(yk::http::HttpRequest::ptr header
                             ,yk::http::WSSession::ptr session) {
    auto id = header->getHeader("$id");
    YK_LOG_INFO(g_logger) << "onClose "<< " id=" << id;
    if(!id.empty()) {
...
int32_t CatChatWSServlet::handle(yk::http::HttpRequest::ptr header
                           ,yk::http::WSFrameMessage::ptr msgx
                           ,yk::http::WSSession::ptr session) {
    YK_LOG_INFO(g_logger) << "handle " << session
            << " opcode=" << msgx->getOpcode()
